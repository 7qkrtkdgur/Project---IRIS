"""마이크 입력 게이지 + 드래그 감도(인식 임계) 막대 (고급 주파수 그라데이션 버전)"""
...
class MicLevelGaugeWidget(QWidget):
    ...
    양쪽 끝이 흐려지며 소리 크기에 따라 위아래로 출렁이는 주파수 효과 적용.
    """
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        # 1. 배경 트랙 그리기 (어둡고 깔끔한 다크모드 배경 박스)
        p.setPen(Qt.PenStyle.NoPen)
        p.setBrush(QColor("#13151a")) # 주변 배경과 어울리는 딥 블랙
        ...
        # 2. 양쪽을 흐리게 만드는 그라데이션 브러시 설정
        gradient = QLinearGradient(inner.left(), 0, inner.right(), 0)
        
        # 소리가 기준치를 넘었는지 여부에 따라 주파수 색상 변경
        base_color = QColor("#22d3ee") if self._level >= self._threshold_display else QColor("#475569")
        
        # 양쪽 끝은 투명하게, 가운데는 선명하게 흐림 효과
        gradient.setColorAt(0.0, QColor(base_color.red(), base_color.green(), base_color.blue(), 0))
        gradient.setColorAt(0.1, QColor(base_color.red(), base_color.green(), base_color.blue(), 255))
        gradient.setColorAt(0.9, QColor(base_color.red(), base_color.green(), base_color.blue(), 255))
        gradient.setColorAt(1.0, QColor(base_color.red(), base_color.green(), base_color.blue(), 0))

        # 3. 위아래로 출렁이는 주파수 파형선 그리기
        level_w = self._display_to_x(self._level, inner) - inner.left()
        if level_w > 0:
            # [수정 완료] 오타 수정: inner.centerY() -> inner.center().y()
            center_y = inner.center().y()
            wave_height = max(2, int(self._level * (inner.height() - 4)))
            
            wave_rect = QRect(inner.left(), center_y - (wave_height // 2), level_w, wave_height)
            
            p.setBrush(gradient)
            p.drawRoundedRect(wave_rect, 4, 4)

        # 4. 노란색 감도 조절 세로 막대 및 삼각형 포인터 그리기
