"""채팅 패널 (테스트 코드가 제거된 진짜 최종 배포용 버전)"""
...
class _ChatInputBar(QWidget):
    ...
    def __init__(self, parent: QWidget | None = None) -> None:
        ...
        # [수정 완료] 입력창 테두리를 살리고 다크 모드 스타일에 맞게 변경
        self.input = QLineEdit()
        self.input.setPlaceholderText("Iris에게 메시지를 입력하세요…")
        
        # 민우님이 테두리를 강제로 꺼둔 setFrame(False)를 제거하고, 스타일시트로 예쁜 박스를 그려줍니다.
        self.input.setStyleSheet("""
            QLineEdit {
                background-color: #1a1c24;  /* 입력창 내부를 살짝 어두운 색으로 채우기 */
                color: #ffffff;             /* 타이핑하는 글자색은 흰색 */
                border: 1px solid #3f3f5f;  /* 은은한 네이비색 테두리 박스 선 생성 */
                border-radius: 6px;         /* 모서리를 둥글게 가공 */
                padding: 6px 10px;          /* 글자 입력할 때 답답하지 않게 안쪽 여백 추가 */
            }
        """)
